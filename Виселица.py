...
word_list = word_rus = ['год', 'человек', 'время', 'дело', 'жизнь', 'день', 'рука', 'раз', 'работа', 'слово', 'место',
            ...
            'начало', 'свет', 'пора', 'путь', 'душа', 'уровень', 'форма', 'связь', 'минута', 'улица', 'вечер', 'конец',
...
def play(word):
    ...
    while True:
        ...
        letter = input(f'Введите {again} букву : ')
        ...
        if letter.isalpha() and letter not in guessed_letters:
            for i in range(len(word)):
                if tries < 2:
                    ...
                    return '''Sorry, you're loose'''

                if tries > 1 and letter == word[i]:
                    word_completion = [i for i in word_completion]
                    word_completion[i] = letter
                    if letter not in guessed_letters:
                        guessed_letters.append(letter)
                    print(''.join(word_completion))
                    print('Список названных букв: ', sorted(guessed_letters))

                if tries > 1 and letter not in word :
                    tries -= 1
                    if letter not in guessed_letters:
                        guessed_letters.append(letter)
                ...
                if word == ''.join(word_completion):
                    print('The word is ', word)
                    return '''You're smart as fuck!'''


def loop():
    repeat = input('Еще разок? Д или Н ')
    if repeat.lower() == 'да':
        word = get_word(word_list)
        play(word)
    if repeat.lower() == 'н':
        exit('Have a good one!')
